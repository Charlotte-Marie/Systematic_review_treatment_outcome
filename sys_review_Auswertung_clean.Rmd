...
## Figure 1a: Visualize differences between balanced and reported accuracies
```{r}
```
...
Risk of data leakage has small effect.
The sample size has a substantial effect on the accuracy.


# Optional RQ 1: Focus on mean (approach before including the meta-analysis)
...
```{r }
ggplot(data_acc_long, aes (x = Study, y = metric, color = time)) +
  geom_point(size = 4) +
  coord_flip()
```
...
# Add weights from the random effects model
data_acc$weight <- m.prop[["w.random"]]
...
plot_acc_contr_sample_size <- ggplot(data = data_acc, aes(y =`Balanced_acc_final`, x = `Sample_size`, size = `weight`))+
    theme(text = element_text(family = "Arial"))+
  geom_point(aes(color = `Treatment`))+ #draw point for all treatments
  ...
  scale_x_continuous(limits=c(0,147), breaks = c(0,25,50,75,100,125,150)) +
    theme(legend.position = "bottom")+
  theme(legend.title=element_blank()) +
  guides(color = guide_legend(nrow = 2, byrow = TRUE))
