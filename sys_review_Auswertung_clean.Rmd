...
## Set working directory to folder that contains excel file "Summary_TR_predictions_with_rs.xlsx"
```

# Import data
```{r Import data tables}
...
## Compress table entries, delete columns and change column names
```{r Compress data_table}
...
# Delete columns age group and year
...
# Save flextable to word (reminder: change path)
margins <- page_mar(
  bottom = 0.5,
  top = 0.5,
  right = 0.5,
...
# Create nice_table (für das html)
nice_table(data_table)
```

# RQ 1: How high is the accuracy of treatment outcome prediction?
...
```{r RQ 1: How well can treatment outcome in internalizing disorders be predicted by features based on resting-state functional connectivities?}

# Add column for ROB/risk of data leakage of the respective study
...
# Safe both plots with sensible defaults
plot_acc_control_violin
...
# Add column for type of treatment manually
...
# Safe plots with sensible defaults
...
# RQ 2.1: Which approaches were used to calculate feature importance?
...
## Plot data
```{r RQ 2.1: Which approaches are taken to draw inferences about the predictive value of specific features?}
...
# RQ 2.2: Which features have predictive value?
## Bring data into long-format
```{r RQ 2.2: Which brain regions are important for the prediction of treatment outcome?}
...
## Change designation of brain regions
```{r Brain regions}
...
```{r Wide data set with absolute and relative frequencies per region}
...
```{r RQ 3 Which approaches are taken to reduce the large amount of theoretically initially available functional connectivities to a small set of features to be used in the final classifier(s)?}
