...
size_text = 10
theme_set(theme_bw() + theme(text = element_text(size =size_text, family = "sans"),
                             axis.title = element_text(size =size_text, family = "sans"), plot.title = element_text(size = size_text, family = "sans"),
                             axis.text = element_text(size = size_text, family = "sans"),
                             legend.text =  element_text(size =size_text-1, family = "sans")))
...
ggsave("plots/violin_acc_normal_and_controlled.eps",plot_acc_contr_violin)
ggsave("plots/violin_acc_normal_and_controlled_rob.eps",plot_acc_contr_violin_rob, height = 4)
```
...
plot_acc_contr_sampel_size <- ggplot(data = data_acc, aes(y =`Accuracy_controlled`, x = `Sample_size`))+
...
# Combine both plots (Accuray with risk of bias and Sample size and balanced accuracy)
plot_combined <- plot_acc_contr_violin_rob + plot_acc_contr_sampel_size + plot_annotation(tag_levels = "A") + plot_layout(ncol=2)

# Display and safe plots with sensible defaults
plot_acc_contr_sampel_size
plot_combined

ggsave("plots/plot_samplesize.eps",plot_acc_contr_sampel_size, height = 4)
ggsave("plots/plot_combined.eps",plot_combined, height = 3.5)
```
...
## Plot feature importance (predicive value in absolute and relative frequencies) 
...
ggsave("plots/plot_feat_pred_value.eps",plot_feat_pred_value_2, height = 4)
```
...
# Safe plot with sensible defaults
plot_PROBAST
ggsave("plots/plot_PROBAST.eps",plot_PROBAST, height = 4.5, width = 9)
```
