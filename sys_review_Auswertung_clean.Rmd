...
size_text = 10
theme_set(theme_minimal() +  # Use a minimal theme (or choose another theme)
  # theme(
  ...
  # ) + 
    theme(text = element_text(size =size_text, family = "Helvetica"),
                             ...
                             legend.text =  element_text(size =size_text, family = "Helvetica")))
...
data_sumtable
```
# RQ 1: Meta-analysis of balanced accuracy values
The mean raw accuracy of the best model per study is  `r round(mean(data_acc$Reported_Accuracy_rounded))`%, with a range of `r min(data_acc$Reported_Accuracy_rounded)`% - `r max(data_acc$Reported_Accuracy_rounded)`%.
...
## Figure 2a: Visualize differences between balanced and raw accuracies
```{r}
...
pdf("plots/figure_3_forestplot.pdf", width = 8, height = 5)
...
## Meta regression with subgroups
```{r}
update(m.prop, 
            subgroup = Treatment, 
            tau.common = FALSE)
...
## Meta regression with sample size (continuous variables)
```{r}
...
## Figure 2b: Sample size and balanced accuracy for each treatment type
...
## Optional: Plot raw and balanced accuracy without (1) and with risk of data leakage (2)
```{r}
...
## Table S1: Approaches to assess predictive value
```{r}
...
## Table S2: Level of resolution of reporting brain regions
```{r}
...
## Figure 4: Plot feature importance (predictive value in absolute and relative frequencies) 
